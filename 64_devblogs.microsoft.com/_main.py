...
url_list = []
base_dir = './devblogs.microsoft.com'
count=1
...
ir=1
try:
    podcast=driver.find_element_by_xpath('//*[@id="most-recent"]')
    a = podcast.find_elements_by_tag_name('a')
    for x in a:
        link = x.get_attribute('href')
        print(link)
        url_list.append(link)
        if link.startswith('https://devblogs.microsoft.com/'):
            url_list.append(link)
        else:
            pass
    time.sleep(3)
    while True:
        driver.find_element_by_xpath('//a[@class="next page-link"]').click()
        time.sleep(5)
        podcast = driver.find_element_by_xpath('//*[@id="most-recent"]')
        a = podcast.find_elements_by_tag_name('a')
        for x in a:
...
for za in url_list:
    print(za)
    if za.startswith('https://devblogs.microsoft.com/'):
        driver.get(za)
        ...
        try:
            ...
            time.sleep(5)
            title1 = driver.find_element_by_xpath('//h1[@class="entry-title x-hidden-focus"]')
            title = title1.text
            print(title)
            ...
            transcript_link=driver.find_element_by_xpath('//div[@class="fsplyr-controls external"]/a[3]')
            ...
            try:
                audio_lnk=driver.find_element_by_xpath('//div[@class="fsplyr-controls external"]/a[1]')
