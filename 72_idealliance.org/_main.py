...
url = 'https://idealliance.org/category/gamut-podcast/'
action = ActionChains(driver)
url_list = []
base_dir = './idealliance.org'
count=1
...
ir=1
try:
    podcast=driver.find_element_by_xpath('//*[@id="main"]/div[3]/div[1]/div[2]')
    a = podcast.find_elements_by_tag_name('a')
    for x in a:
        link = x.get_attribute('href')
        print(link)
        if link.startswith('https://idealliance.org/'):
...
for za in url_list:
    ...
    if za==None:
        ...
        try:
            ...
            time.sleep(5)
            title1 = driver.find_element_by_xpath('//h1[@class="h2 post_title"]')
            title = title1.text

            print(title)

            date_ = driver.find_element_by_xpath('//li[@class="post_date h6"]')
            date_ = date_.text
            from dateutil.parser import parse
            ...
            transcript=driver.find_element_by_xpath('//div[@class="stm_single_post"]/article/div/div[1]/div/div/section/div/div/div[1]/div/div/div[2]/div/div')
            ...
            try:
                iframe=driver.find_element_by_xpath('//div[@class="elementor-text-editor elementor-clearfix"]/p[1]/iframe')
                ...
                print(audio_lnk,'audio link')
                driver.get(str(audio_lnk))
                time.sleep(4)
