...
url = 'https://www.newchurches.com/podcasts/new-churches-podcast'
action = ActionChains(driver)
url_list = []
base_dir = './newchurches.com'
count=1
...
ir=1
try:
    podcast=driver.find_element_by_xpath('//div[@class="uabb-module-content uabb-blog-posts uabb-blog-posts-grid uabb-post-grid-1 "]')
    a = podcast.find_elements_by_tag_name('a')
    for x in a:
        link = x.get_attribute('href')
        print(link)
        if link.startswith('https://www.newchurches.com'):
            url_list.append(link)
    time.sleep(3)
    while True:
        nextpage=driver.find_element_by_xpath('//a[@class="next page-numbers"]')
